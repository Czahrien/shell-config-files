...
### Git [±master ▾●]
# NOTE: Something about OpenBSD's zsh 5.9 counts the unicode characters we are using here as two characters.
ZSH_THEME_GIT_PROMPT_PREFIX="[%{$fg_bold[green]%}%{±%G%}%{$fg_bold[white]%}"
ZSH_THEME_GIT_PROMPT_SUFFIX="%{$reset_color%}]"
ZSH_THEME_GIT_PROMPT_CLEAN="%{$fg_bold[green]%}%{✓%G%}%{$reset_color%}"
ZSH_THEME_GIT_PROMPT_AHEAD="%{$fg[cyan]%}%{▴%G%}%{$reset_color%}"
ZSH_THEME_GIT_PROMPT_BEHIND="%{$fg[magenta]%}%{▾%G%}%{$reset_color%}"
ZSH_THEME_GIT_PROMPT_STAGED="%{$fg_bold[green]%}%{●%G%}%{$reset_color%}"
ZSH_THEME_GIT_PROMPT_UNSTAGED="%{$fg_bold[yellow]%}%{●%G%}%{$reset_color%}"
ZSH_THEME_GIT_PROMPT_UNTRACKED="%{$fg_bold[red]%}%{●%G%}%{$reset_color%}"
...
bureau_git_prompt() {
  ...
  # off by one
  #echo "${ZSH_THEME_GIT_PROMPT_PREFIX}TESTING${ZSH_THEME_GIT_PROMPT_SUFFIX}"
  #echo "±±±"
  #echo "${output}"
}
...
bureau_precmd () {
  _1RIGHT="[%{$fg_bold[yellow]%}$(${HOME}/utils/loadavg)%{$reset_color%}][%*]"
...
DEBUG_FILE=${HOME}/wtf.out
echo "$(bureau_git_prompt)" > $DEBUG_FILE
